...
tests/test-sim-analy-irbssfp: sim nrmse
	...
	$(TOOLDIR)/sim -P 1:1:0.0045:0.00225:0.001:45:1000 analytical.ra	;\
	$(TOOLDIR)/sim -P 0:1:0.0045:0.00225:0.00001:45:1000 simu.ra		;\
...
tests/test-sim-analy-flash: sim cabs mip spow fmac nrmse
	set -e; mkdir $(TESTS_TMP) ; cd $(TESTS_TMP)						;\
	$(TOOLDIR)/sim -P 1:2:0.0041:0.00258:0.001:6:1000 analytical.ra		;\
	$(TOOLDIR)/sim -P 0:2:0.0041:0.00258:0.001:6:1000 _simu.ra			;\
...
tests/test-sim-analy-irflash: sim cabs mip spow fmac nrmse
	set -e; mkdir $(TESTS_TMP) ; cd $(TESTS_TMP)						;\
	$(TOOLDIR)/sim -P 1:5:0.0041:0.00258:0.001:6:1000 analytical.ra		;\
	$(TOOLDIR)/sim -P 0:5:0.0041:0.00258:0.001:6:1000 _simu.ra			;\
	$(TOOLDIR)/cabs _simu.ra _simu_abs.ra								;\
	$(TOOLDIR)/mip 32 _simu_abs.ra max.ra								;\
	$(TOOLDIR)/spow -- -1 max.ra scale.ra								;\
	$(TOOLDIR)/fmac _simu.ra scale.ra simu.ra							;\
...
tests/test-sim-ode-bssfp: sim nrmse
	set -e; mkdir $(TESTS_TMP) ; cd $(TESTS_TMP)						;\
	$(TOOLDIR)/sim -P 0:0:0.0045:0.00225:0.001:45:1000 simu_obs.ra		;\
	$(TOOLDIR)/sim -o -P 0:0:0.0045:0.00225:0.001:45:1000 simu_ode.ra	;\
...
tests/test-sim-ode-irbssfp: sim nrmse
	set -e; mkdir $(TESTS_TMP) ; cd $(TESTS_TMP)						;\
	$(TOOLDIR)/sim -P 0:1:0.0045:0.00225:0.001:45:1000 simu_obs.ra		;\
	$(TOOLDIR)/sim -o -P 0:1:0.0045:0.00225:0.001:45:1000 simu_ode.ra	;\
...
tests/test-sim-ode-flash: sim nrmse
	set -e; mkdir $(TESTS_TMP) ; cd $(TESTS_TMP)						;\
	$(TOOLDIR)/sim -P 0:2:0.0041:0.00258:0.001:6:1000 simu_obs.ra		;\
	$(TOOLDIR)/sim -o -P 0:2:0.0041:0.00258:0.001:6:1000 simu_ode.ra	;\
...
tests/test-sim-ode-irflash: sim nrmse
	set -e; mkdir $(TESTS_TMP) ; cd $(TESTS_TMP)						;\
	$(TOOLDIR)/sim -P 0:5:0.0041:0.00258:0.001:6:1000 simu_obs.ra		;\
	$(TOOLDIR)/sim -o -P 0:5:0.0041:0.00258:0.001:6:1000 simu_ode.ra	;\
