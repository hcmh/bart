...
tests/test-nufft-forward-no-mem: traj phantom nufft nrmse
	set -e ; mkdir $(TESTS_TMP) ; cd $(TESTS_TMP)					;\
	$(TOOLDIR)/traj -r -x128 -y128 traj.ra						;\
	$(TOOLDIR)/phantom -s4 phan.ra							;\
	$(TOOLDIR)/nufft    -r traj.ra phan.ra ksp1.ra					;\
	$(TOOLDIR)/nufft --no-mem traj.ra phan.ra ksp2.ra				;\
	$(TOOLDIR)/nrmse -t 0.00001 ksp1.ra ksp2.ra					;\
	rm *.ra ; cd .. ; rmdir $(TESTS_TMP)
	touch $@
...
tests/test-nufft-lowmem-no-mem: zeros noise traj nufft nrmse
	set -e; mkdir $(TESTS_TMP) ; cd $(TESTS_TMP)					;\
	$(TOOLDIR)/zeros 4 1 128 128 3 z.ra						;\
	$(TOOLDIR)/noise -s321 z.ra n2.ra						;\
	$(TOOLDIR)/traj -r -x128 -y128 traj.ra						;\
	$(TOOLDIR)/nufft -a traj.ra n2.ra x1.ra						;\
	$(TOOLDIR)/nufft --no-mem -a traj.ra n2.ra x2.ra				;\
	$(TOOLDIR)/nrmse -t 0.000001 x1.ra x2.ra					;\
	rm *.ra ; cd .. ; rmdir $(TESTS_TMP)
	touch $@
...
TESTS += tests/test-nufft-lowmem-no-mem tests/test-nufft-lowmem-no-mem
