 ...
 * 
 * Manuscript:
 * Sebastian Rosenzweig, Nick Scholand, H. Christian M. Holme, Martin Uecker. Cardiac and Respiratory Self-Gating in Radial MRI using an Adapted Singular Spectrum Analysis (SSA-FARY)
 *
 *
 */
...
int main_rmfreq(int argc, char* argv[])
{
	...
	for (int i = 0; i < N; i++) {
		angles[i] = M_PI + atan2f(crealf(t1[3 * i + 0]), crealf(t1[3 * i + 1])) + 0. * I;
