...
int main_genLLbasis(int argc, char* argv[argc])
{
	...
	int nspokes   = 3;
        int nTI       = 51;
        float t;
	float T1s_max = 5000e-3;
	float T1s_min = 5e-3;
	
	float Mss_max = 1000.0e-3;
	float Mss_min = 10.0e-3;
        
        // number of R1s and Mss in the dictionary
        int nR1s = 1000;
        int nMss = 100;
...
// 
	...
	md_copy_dims(DIMS, odims, dims_R1s);
        ...
        long odims_strs[DIMS];
	md_calc_strides(DIMS, odims_strs, odims, CFL_SIZE);
        
        long dims_R1s_strs[DIMS];
	md_calc_strides(DIMS, dims_R1s_strs, dims_R1s, CFL_SIZE);
        
        long dims_Mss_strs[DIMS];
	md_calc_strides(DIMS, dims_Mss_strs, dims_Mss, CFL_SIZE);
