...
static void rand_mask_fixed_fun(const nlop_data_t* _data, int D, complex float* args[D])
{
	...
	do {
		...
		for (int i = 0; i < ones; i++) {
			...
			md_unravel_index(N, pos, ~data->bat_flags, data->dims, rand_r(&(data->seed)) % (NV - i));

			while (1. == MD_ACCESS(N, strs, pos, data->state))
				if (!md_next(N, data->dims, ~data->bat_flags, pos))
					assert(0);
...
const struct nlop_s* nlop_rand_split_fixed_create(int N, const long dims[N], unsigned long shared_dims_flag, unsigned long bat_dims_flag, float p, unsigned long fix_flags, complex float* _fix_first)
{
	long fix_dims[N];
	md_select_dims(N, fix_flags, fix_dims, dims);

	complex float* fix_first = md_alloc(N, fix_dims, CFL_SIZE);
	...
	auto result = nlop_chain2_FF(invert, 0, nlop_tenmul_create(N, dims, dims, dims2), 1);
	result = nlop_combine_FF(result, nlop_tenmul_create(N, dims, dims, dims2));
	result = nlop_dup_F(result, 0, 2);
	result = nlop_dup_F(result, 1, 2);

	result = nlop_prepend_FF(nlop_from_linop_F(linop_fmac_create(N, dims2, 0, shared_dims_flag, ~fix_flags, fix_first)), result, 1);
