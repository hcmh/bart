...
static const struct nlop_s* get_nn_mnist_default(int N_batch)
{
	unsigned int N = 5;
...
static const struct nlop_s* get_nn_mnist_batchnorm(int N_batch)
{
	unsigned int N = 5;
	long indims[] = {1, 28, 28, 1, N_batch};
	long expdims[] = {2, 28, 28, 1, N_batch};
	...
	network = append_convcorr_layer(network, 0, 32, kernel_size, conv, PAD_VALID, true, NULL, NULL);
	network = append_batchnorm_layer(network, 0, ~MD_BIT(0));
	network = append_activation_bias(network, 0, ACT_RELU, MD_BIT(0));
	network = append_convcorr_layer(network, 0, 64, kernel_size, conv, PAD_VALID, true, NULL, NULL);
	network = append_batchnorm_layer(network, 0, ~MD_BIT(0));
	network = append_activation_bias(network, 0, ACT_RELU, MD_BIT(0));
...
const struct nlop_s* get_nn_mnist(enum MNIST_NETWORK_TYPE type, int N_batch)
{
	switch (type) {

		case MNIST_NETWORK_DEFAULT:
			return get_nn_mnist_default(N_batch);

		case MNIST_NETWORK_BATCHNORM:
			return get_nn_mnist_batchnorm(N_batch);
	}

	assert(0);
}

int nn_mnist_get_num_weights(enum MNIST_NETWORK_TYPE type)
{
	const struct nlop_s* network = get_nn_mnist(type, 1);
...
void init_nn_mnist(enum MNIST_NETWORK_TYPE type, complex float* weights)
{
	const struct nlop_s* network = get_nn_mnist(type, 1);

	for (int i = 0; i < nlop_get_nr_in_args(network); i++)
		if (i != 0)
            		weights = init_auto_nlop_props(network, i, weights);
...
void train_nn_mnist(enum MNIST_NETWORK_TYPE type, int N_batch, int N_total, complex float* weights, const complex float* in, const complex float* out, long epochs)
{
	...
	const struct nlop_s* network = get_nn_mnist(type, N_batch);
	...
	for (int i = 0; i < NI; i++) {
		...
		bool bn = OP_PROP_NN_BATCH_NORM == nlop_get_input_weight_type(nlop_get_ii_props(nlop_train, i, i));
		in_type[i] = bn ? IN_BATCHNORM : IN_OPTIMIZE;
	}

	enum OUT_TYPE out_type[NO];
	for (int o = 0; o < NO; o++) {

		bool bn = OP_PROP_NN_BATCH_NORM == nlop_get_input_weight_type(nlop_get_oo_props(nlop_train, o, o));
		out_type[o] = bn ? OUT_BATCHNORM : OUT_OPTIMIZE;
	}
...
void predict_nn_mnist(enum MNIST_NETWORK_TYPE type, int N_total, int N_batch, long prediction[N_total], const complex float* weights, const complex float* in)
{
...
float accuracy_nn_mnist(enum MNIST_NETWORK_TYPE type, int N_total, int N_batch, const complex float* weights, const complex float* in, const complex float* out)
{
	long prediction[N_total];
	predict_nn_mnist(type, N_total, N_batch, prediction, weights, in);
