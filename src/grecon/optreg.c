...
unsigned int llr_blk, unsigned int shift_mode, bool use_gpu)
{
	return opt_reg_configure2(N, img_dims, ropts, prox_ops, trafos, llr_blk, shift_mode, NULL, NULL, use_gpu);
}

void opt_reg_configure2(unsigned int N, const long img_dims[N], struct opt_reg_s* ropts, const struct operator_p_s* prox_ops[NUM_REGS], const struct linop_s* trafos[NUM_REGS],
