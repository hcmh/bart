...
extern void noir2_net_get_col_dims(struct noir2_net_s* x, int N, long img_dims[N]);
...
extern const struct nlop_s* noir_l2_loss_create(struct noir2_net_s* model, float temp_damping, _Complex float* mask);
extern const struct nlop_s* noir_rtnlinv_avg_coils_create(struct noir2_net_s* model, int window);
