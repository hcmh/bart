...
struct noir2_model_conf_s {
	...
	unsigned long fft_flags_noncart;
	unsigned long fft_flags_cart;
	unsigned long fft_flags_centered;
	unsigned long fft_flags_unitary;
...
struct noir2_s {
	...
	long* ffm_dims;
	struct multiplace_array_s* nufft_weighting;
};
...
extern void noir2_orthogonalize(int N, const long col_dims[N], int idx, unsigned long flags, _Complex float* coils);