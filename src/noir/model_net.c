	...
	struct noir2_model_conf_s* model_conf)
{
	...
	x->trj_dims = ARR_CLONE(long[N], trj_dims);
	x->wgh_dims = ARR_CLONE(long[N], wgh_dims);
	x->bas_dims = ARR_CLONE(long[N], bas_dims);
	x->msk_dims = ARR_CLONE(long[N], msk_dims);
	x->ksp_dims = ARR_CLONE(long[N], ksp_dims);
	x->cim_dims = ARR_CLONE(long[N], cim_dims);
	x->img_dims = ARR_CLONE(long[N], img_dims);
	x->col_dims = ARR_CLONE(long[N], col_dims);
...
struct noir2_net_s* noir2_net_create(struct noir2_net_config_s* config, int NB)
{
	...
	for (long i = 0; i < NB * NB_model; i++) {
		...
		if (config->noncart)
			models[i] = noir2_noncart_create(N, trj_dims, NULL, wgh_dims, NULL, bas_dims, config->basis, msk_dims, config->mask, ksp_dims, cim_dims, img_dims, col_dims, &(config->mconf));
		else
			models[i] = noir2_cart_create(N, wgh_dims, NULL, bas_dims, config->basis, msk_dims, config->mask, ksp_dims, cim_dims, img_dims, col_dims, &(config->mconf));		
	}
...
void noir2_net_get_col_dims(struct noir2_net_s* x, int N, long col_dims[N])
{
	assert(N == x->config->N);
	md_copy_dims(N, col_dims, x->config->col_dims);
}
...
const struct nlop_s* noir_coil_rss_create(struct noir2_net_s* model)
{
	auto ret = noir_decomp_create(model);
	ret = nlop_del_out_F(ret, 0);
	
	auto cod = nlop_codomain(ret);
	ret = nlop_chain_FF(ret, nlop_zrss_create(cod->N, cod->dims, COIL_FLAG));
...
const struct nlop_s* noir_l2_loss_create(struct noir2_net_s* model, float temp_damping, complex float* temp_mask)
{
	...
	if (0 < temp_damping) {
		...
		nlop_l2 = nlop_chain2_keep_FF(nlop_l2, 1, nlop_from_linop_F(linop_shift_create(N, col_dims, TIME_DIM, 1, PAD_CAUSAL)), 0);
		nlop_l2 = nlop_chain2_keep_FF(nlop_l2, 1, nlop_from_linop_F(linop_shift_create(N, img_dims, TIME_DIM, 1, PAD_CAUSAL)), 0);

		nlop_l2 = nlop_no_der_F(nlop_l2, 0, 0);
		nlop_l2 = nlop_no_der_F(nlop_l2, 1, 0);

		nlop_l2 = nlop_chain2_FF(nlop_l2, 1, nlop_zaxpbz_create(N, col_dims, -0.9, 1), 0);
		nlop_l2 = nlop_link_F(nlop_l2, 3 , 0);

		nlop_l2 = nlop_chain2_FF(nlop_l2, 1, nlop_zaxpbz_create(N, img_dims, -0.9, 1), 0);
		nlop_l2 = nlop_link_F(nlop_l2, 2 , 0);
	}

	if (NULL != temp_mask) {

		nlop_l2 = nlop_append_FF(nlop_l2, 0, nlop_from_linop_F(linop_cdiag_create(N, img_dims, TIME_FLAG, temp_mask)));
		nlop_l2 = nlop_append_FF(nlop_l2, 1, nlop_from_linop_F(linop_cdiag_create(N, col_dims, TIME_FLAG, temp_mask)));
	}

	nlop_l2 = nlop_append_FF(nlop_l2, 0, nlop_zss_create(N, img_dims, ~BATCH_FLAG));	
	nlop_l2 = nlop_append_FF(nlop_l2, 1, nlop_zss_create(N, col_dims, ~BATCH_FLAG));

	nlop_l2 = nlop_chain2_FF(nlop_l2, 0, nlop_zaxpbz_create(N, bat_dims, 1., 1.), 0);
	nlop_l2 = nlop_link_F(nlop_l2, 1, 0);

	return nlop_l2;
}

const struct nlop_s* noir_rtnlinv_avg_coils_create(struct noir2_net_s* model, int window)
{
	auto nlop_avg_coil = noir_split_create(model);

	auto cod = nlop_generic_codomain(nlop_avg_coil, 1);
	int N = cod->N;
	long col_dims[N];
	md_copy_dims(N, col_dims, cod->dims);

	int window_dim = TIME_DIM;
	while (1 != col_dims[++window_dim])
		assert(BATCH_DIM > 1);

	const struct linop_s* lop_prep = linop_padding_create_onedim(N, col_dims, PAD_CAUSAL, TIME_DIM, window - 1, 0);
	lop_prep = linop_chain_FF(lop_prep, linop_hankelization_create(N, linop_codomain(lop_prep)->dims, TIME_DIM, window_dim, window));
	lop_prep = linop_chain_FF(lop_prep, linop_avg_create(N, linop_codomain(lop_prep)->dims, MD_BIT(window_dim)));

	nlop_avg_coil = nlop_append_FF(nlop_avg_coil, 1, nlop_from_linop_F(lop_prep));
	nlop_avg_coil = nlop_chain2_FF(nlop_avg_coil, 0, noir_join_create(model), 0);

	nlop_avg_coil = nlop_link_F(nlop_avg_coil, 1, 0);

	return nlop_avg_coil;
}
