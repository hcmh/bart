...
static float compute_h(float noise_level, float begin_sigma, float end_sigma, unsigned int nr_classes)
{
	double step = (log(begin_sigma)-log(end_sigma))/(nr_classes - 1);
	return round((log(begin_sigma)-log(noise_level))/step);
}
...
extern const struct operator_p_s* prox_logp_create(unsigned int N, const long dims[__VLA(N)], const struct nlop_s * tf_ops, float step_size, float p, unsigned int steps, unsigned int prior_dim)
{
...
static void prox_logp_ncsn_fun(const operator_data_t* data, float lambda, complex float *dst, const complex float* src)
{
	auto pdata = CAST_DOWN(prox_logp_ncsn_data, data);
	...
	// generate noise
	complex float* noise = md_alloc(cod->N, cod->dims, cod->size);
	md_gaussian_rand(cod->N, cod->dims, noise);
...
static void prox_logp_ncsn_apply(const operator_data_t* data, float lambda, complex float *dst, const complex float* src)
{
	...
	if(pdata->get_lambda)
	{
		pdata->lambda = lambda / pdata->begin_sigma;
		pdata->get_lambda = false;
	}
	...
	for(unsigned int i=0; i<pdata->steps; i++)
	{
		...
		prox_logp_ncsn_fun(data, lambda, dst, tmp);
		md_copy(pdata->N, pdata->dims, tmp, dst, CFL_SIZE);
	}
...
static void prox_logp_ncsn_del(const operator_data_t* _data)
{
...
extern const struct operator_p_s* prox_logp_nlinv_create(unsigned int N, const long dims[__VLA(N)], const struct nlop_s * tf_ops, float step_size, float p, unsigned int steps, float base, unsigned int irgnm_steps, float rho, unsigned int prior_dim)
{
