...
static void dft(unsigned int N, unsigned int M, complex float out[N], const complex float buf[M])
{
...
static void dftH(unsigned int N, unsigned int M, complex float out[N], const complex float buf[M])
{
	for (unsigned int i = 0; i < N; i++) {
...
void tse(unsigned int N, complex float out[N], unsigned int M, const float in[4])
{
	complex float buf[M];
...
void tse_der(unsigned int N, complex float out[N], unsigned int M,
...
void tse_adj(unsigned int N, float out[4], unsigned int M,
	float in[4],
	complex float inb[N])
{
	complex float buf[M];
