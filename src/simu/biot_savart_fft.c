...
/**
 * Calculate the z-component of the magnetic induction B
...
/**
 * Wrapper for linop_bz_create: J (A/mm^2) -> B (Hz)
 ...
 */
void biot_savart_fft(const long *dims, const float *vox, const complex float *j, complex float *b)
{
	assert(dims[0] == 3 || dims[0] == 2);
	long odims[NB] = {1, dims[1], dims[2], dims[3]};
	auto bz = linop_bz_create(dims, vox);
	complex float *j_scaled = md_alloc(4, dims, CFL_SIZE);

	md_zsmul(4, dims, j_scaled, j, bz_unit(dims+1, vox) * 1e6 );
	linop_forward(bz, NB, odims, b, NB, dims, j_scaled);
	md_zsmul(4, dims, b, b, Hz_per_Tesla * Mu_0);
