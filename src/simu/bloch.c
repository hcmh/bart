...
void bloch_b1_pdp(float out[3][3], const float in[3], float r1, float r2, const float gb[3], float phase)
{
	(void)r1; (void)r2; (void)gb;
	...
	out[1][1] = -in[1];
	out[1][2] = 0.;
	out[2][0] = -sinf(phase) * in[2];
	out[2][1] = cosf(phase) * in[2];
	out[2][2] = sinf(phase) * in[0] - cosf(phase) * in[1];
}
