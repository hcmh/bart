...
int main_T1fun(int argc, char* argv[argc])
{
	...
	struct arg_s args[] = {
		...
		ARG_INFILE(true, &param_file, "parameter maps"),
		ARG_INFILE(true, &time_file, "time"),
	...
	complex float* para_maps = load_cfl(param_file, DIMS, maps_dims);
	complex float* TI = load_cfl(time_file, DIMS, TI_dims);
