 ...
 * Copyright 2017. Sebastian Rosenzweig.
...
static const char usage_str[] = "<kspace> <traj> <output> ";
...
int main_cordelay(int argc, char* argv[])
{
	...
	long traj_dims[DIMS];
	complex float* traj = load_cfl(argv[2], DIMS, traj_dims);

	md_check_compat(DIMS, ~(READ_FLAG|COIL_FLAG), k_dims, traj_dims);


	// Choose only a single RO sample for angle calculation
	...
	md_select_dims(DIMS, PHS2_FLAG|TIME_FLAG|SLICE_FLAG, angles_dims, k_dims);
	...
	// Calculate projection 'angles' and corresponding shifts 'dk'
	unsigned long N = k_dims[PHS2_DIM] * k_dims[TIME_DIM] * k_dims[SLICE_DIM];
