...
static void T2_der(const nlop_data_t* _data, complex float* dst, const complex float* src)
{
    ...
    // z' = -rho.*scaling_z.*TI.*exp(-TI.*scaling_z.*z)
    md_zsmul(data->N, data->out_dims, data->tmp_data, data->tmp_expt, -1*data->scaling_z);
    md_zmul2(data->N, data->out_dims, data->out_strs, data->tmp_data, data->map_strs, data->rho, data->out_strs, data->tmp_data);
...
static void T2_adj(const nlop_data_t* _data, complex float* dst, const complex float* src)
{
    ...
    // conj(rho') * src
    md_zconj(data->N, data->out_dims, data->tmp_data, data->tmp_exp);
    ...
    md_clear(data->N, data->map_dims, data->tmp_map, CFL_SIZE); 
    md_zadd2(data->N, data->out_dims, data->map_strs, data->tmp_map, data->map_strs, data->tmp_map, data->out_strs, data->tmp_data);
    md_zimag(data->N, data->map_dims, data->tmp_map, data->tmp_map);
